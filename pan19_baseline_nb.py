...
def baseline(path,outpath,n=3,ft=5,pt=0.1):
    ...
    for index,problem in enumerate(problems):
        ...
        #pos-tagging
        #train_texts = pos_tagging(train_texts)
        ...
        #count vectorizer
        vectorizer_char = CountVectorizer(analyzer='char',ngram_range=(1,n), min_df=4, max_df=10, lowercase=True, vocabulary=vocabulary) #
        vectorizer_word = CountVectorizer(analyzer = 'word', ngram_range=(1,n), min_df=4, max_df=10, lowercase=True) #
                
        #tfidf 
        vectorizer_tfidf_word = TfidfVectorizer(analyzer = 'word', ngram_range = (1,n), use_idf=True, lowercase = True)
        vectorizer_tfidf_char = TfidfVectorizer(analyzer = 'char', ngram_range = (1,n), use_idf=True, lowercase = True, vocabulary = vocabulary)
        #Vi kan bruge en anden vectorizer men så skal den concatenates med en løsning i scikit-learn pakken
        
        vectorizer = FeatureUnion([('vectorizer_tfidf_char', vectorizer_tfidf_char), ('vectorizer_tfidf_word', vectorizer_tfidf_word), ('apo', apo)])

        """
        vectorizer = FeatureUnion([('vectorizer_char', vectorizer_char), ('vectorizer_word', vectorizer_word),
                                ('text_length', text_length), ('sentence_length', sentence_length), ('word_length', word_length), ('lex', lex),
                                ('digits', digits), ('space', space), ('punctuation', punctuation), ('uppercase', uppercase), ('apo', apo),
                                ('quote', quote), ('paren', paren), ('adjective', adjective), ('personal', personal), ('foreign_word', foreign_word)])
        """


        #vectorizer = CountVectorizer(analyzer='word', ngram_range=(1, n), lowercase=False, vocabulary=vocabulary)
        ...
        #pos tagging
        #test_texts = pos_tagging(test_texts)
        ...
        clf=CalibratedClassifierCV(OneVsRestClassifier(MultinomialNB()))
...
def main():
    ...
    evaluate_all('exam_data', 'exam_eval', 'exam_preds')
