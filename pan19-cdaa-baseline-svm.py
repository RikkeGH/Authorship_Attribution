...
def stemmer(texts):
    ...
    for (text, label) in texts:
        ...
        texts_stem.append((str(text), label))
    return texts_stem
...
def baseline(path,outpath,n=3,ft=5,pt=0.1):
    ...
    for index,problem in enumerate(problems):
        ...
        #train_docs = stemmer(train_docs)
        #train_docs = lemmatizer(train_docs)
        ...
        text_length = FunctionTransformer(get_text_length, validate=False) #didnt make a difference with count char & word
                
        #vectorizer = CountVectorizer(analyzer='char',ngram_range=(1,n),lowercase=False, vocabulary=vocabulary)
        
        vectorizer_char = CountVectorizer(analyzer='char',ngram_range=(1,n),lowercase=True, vocabulary=vocabulary) 
        vectorizer_word = CountVectorizer(analyzer = 'word', ngram_range=(1,n), lowercase=True)
                
        #tf-idf
        #vectorizer_tfidf_word = TfidfVectorizer(analyzer = 'word', ngram_range = (1,n), use_idf=True, lowercase = False)
        

        vectorizer = FeatureUnion([("lex", lex), ("foreign word", foreign_word), ("personal", personal), 
                      ("punctuation", punctuation), ("uppercase", uppercase), ('word_length', word_length),
                      ('vectorizer_char', vectorizer_char), ('vectorizer_word', vectorizer_word)])
        ...
        #test_docs = stemmer(test_docs)
        #test_docs = lemmatizer(test_docs)
...
def main():
    ...
    evaluate_all('Pan/exam_data', 'Pan/exam_eval', 'Pan/exam_preds')
