...
def eval_measures(gt, pred):
    ...
    with warnings.catch_warnings():
        ...
        for x in labels:
            if encoder.inverse_transform([x])[0]=='<UNK>':
                labels.remove(x)
...
def evaluate_all(path_collection,path_answers,path_out):
    ...
    for problem in problems:
        ...
        data.append({'problem-name': problem, 'macro-f1': round(f1,3), 'macro-precision': round(precision,3), 'macro-recall': round(recall,3), 'accuracy': round(accuracy,3)})
        print(str(problem),'Macro-F1:',round(f1,3))
    overall_score=sum(scores)/len(scores)
    overall_accuracy = sum(scores_accuracy)/len(scores_accuracy)
    # Saving data to output files (out.json and evaluation.prototext)
    with open(path_out+os.sep+'out.json', 'w') as f:
        json.dump({'problems': data, 'overall_score': round(overall_score,3), 'overall_accuracy': round(overall_accuracy,3)}, f, indent=4, sort_keys=True)
